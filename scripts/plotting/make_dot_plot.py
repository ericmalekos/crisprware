...
def main():
    parser = argparse.ArgumentParser(
        description="Create a dot and violin plot for specified columns based on value counts greater than 0.2, excluding rows with -1, perform Mann-Whitney U test on the full dataset, and save the plot to a file."
    )
    ...
    parser.add_argument(
        "--sample_size", type=int, help="Maximum number of samples to include in the plot for each column (optional)"
    )
    parser.add_argument("--output", type=str, help="The file path to save the plot image")
    parser.add_argument(
        "--title", type=str, default="Dot and Violin Plot of Counts by Column", help="Title for the plot"
    )
