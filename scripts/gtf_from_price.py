...
def load_gtf_data(gtf_file):
    ...
    return pd.read_csv(
        gtf_file, sep="\t", comment="#", header=None, names=gtf_columns, quoting=3, dtype={"start": int, "end": int}
    )
    ...
    with open(output_gtf_path, "w") as out_file:
        for _, price_row in price_df.iterrows():
            ...
            transcript_gtf = gtf_df[
                (gtf_df["transcript_id"] == tscript_id.split("_")[0]) & (gtf_df["feature"] == "transcript")
            ]
            ...
            if not transcript_gtf.empty:
                ...
                exon_entries = gtf_df[
                    (gtf_df["transcript_id"] == tscript_id.split("_")[0]) & (gtf_df["feature"] == "exon")
                ]
