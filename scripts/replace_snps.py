    ...
    for snp_file_path in snp_files:
        ...
        for index, row in snp_df.iterrows():
            ...
            if pd.isna(row["Coordinate (GRCm39)"]):
                print(
                    f"Warning: NaN value found for coordinate in {snp_file_path} at index {index}. Skipping this entry."
                )
                continue
            ...
            if nucleotide not in {"A", "T", "C", "G"}:
                print(
                    f"Warning: Unexpected value '{nucleotide}' at position {pos} in {snp_file_path}. Skipping this entry."
                )
                continue
        ...
        for (chromosome, pos), nucleotide in snps_dict.items():
            if chromosome == current_chromosome:
                try:
                    ...
                    print(
                        f"An error occurred while setting position {pos} to '{nucleotide}' in chromosome {chromosome}: {e}"
                    )
...
if len(sys.argv) < 4:
    print(
        "Usage: python replace_snps.py <path_to_fasta> <path_to_output_fasta> <path_to_snp_file1> [<path_to_snp_file2> ...]"
    )
    sys.exit(1)
