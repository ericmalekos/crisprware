...
parser = argparse.ArgumentParser(
    description="Find a specified window with the highest average signal in a BigWig file for each entry in a BED file."
)
...
with open(args.bed_file, "r") as bed, open(args.output_bed_file, "w") as out_bed:
    for line in bed:
        ...
        highest_start, highest_end, max_avg = find_highest_signal_window(
            bw, chrom, start, end, args.window_size, bigwig_chroms[chrom]
        )
