...
def parse_arguments():
    ...
    parser.add_argument(
        "--select_based_on",
        required=True,
        choices=["AALen", "InFrameCount", "TISQvalue", "FrameQvalue", "FisherQvalue"],
        help="Column to select the best row for each Tid, TisType pair",
    )
...
def filter_and_select_data(df, args):
    ...
    best_rows = filtered_df.sort_values(by=args.select_based_on, ascending=False).drop_duplicates(
        subset=["Tid", "TisType"]
    )
...
def process_and_sort_gtf(df, input_gtf_path, output_gtf_path):
    ...
    for _, ribo_row in df.iterrows():
        for block in ribo_row["Blocks"].split(","):
            ...
            attributes = (
                f'gene_id "{ribo_row["Gid"]}"; transcript_id "{ribo_row["Tid"]}"; gene_name "{ribo_row["Symbol"]}";'
            )
