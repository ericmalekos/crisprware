...
def count_isoforms(gtf_file, protein_coding_only):
    ...
    gene_isoform_count = {
        gene: {"count": len(info["isoforms"]), "gene_name": info["gene_name"]} for gene, info in gene_info.items()
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Rank genes by the number of unique isoforms from a GTF file, with an option to include only protein-coding transcripts. Outputs gene ID, gene name, and the number of isoforms."
    )
    parser.add_argument("gtf_file", help="Path to the GTF file")
    parser.add_argument(
        "--protein_coding", action="store_true", help="Include only protein-coding transcripts in the count"
    )
