...
def process_bed_file(input_file):
    ...
    df = pd.read_csv(
        input_file, sep="\t", comment="#", header=0, names=["chr", "start", "stop", "id", "context", "strand"]
    )
...
def main():
    ...
    parser.add_argument(
        "--reverse", action="store_true", default=False, help="Reverse the sequence before outputting (default: False)"
    )
