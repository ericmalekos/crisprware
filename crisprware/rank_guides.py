...
def save_output(
    ...
    if args.output_all:
        finalgRNAs.to_csv(out_path, sep="\t", index=False)
...
def main(args: Optional[argparse.Namespace] = None) -> None:
    ...
    if args.filtering_columns and (
        not args.minimum_values or (len(args.filtering_columns) != len(args.minimum_values))
    ):
        raise ValueError("\n\n\tThe length of --filtering_columns and --minimum_values must match.\n\n")

    gRNA_output_path, tmp_path = create_output(
        args.scored_guides, outdir=args.output_directory, extension="rankedgRNA", stripped="_scoredgRNA", tmp=True
    )
    gRNA_output_path += ".bed"

    with open(args.scored_guides, "r") as file:
    ...
    if targetFileType in ["gff", "gtf", "gff3", "gff2", "gff.gz", "gtf.gz", "gff3.gz", "gff2.gz"]:
        ...
        finalgRNAs, targetIDs = gRNA_to_tscript(
            gRNAs=gRNAs, mode=args.target_mode, targets=pyb.BedTool(args.targets), header=header
        )
        ...
        trimmed_gtf, transcript_ids, gene_ids = truncate_gtf(
            input_file=args.targets, feature=args.feature, percentiles=args.percentile_range
        )

        if args.target_mode == "tx":
            initial_target_ids = transcript_ids
        else:
            initial_target_ids = gene_ids
        # print(f'\n\tTarget count after {args.feature} trimmming:\t{len(initial_target_ids)}');

        trimmed_gtf.to_csv(
            trimmed, sep="\t", header=False, index=False, doublequote=False, quoting=3, quotechar="", escapechar="\\"
        )
        ...
        if targetFileType not in ["bed"]:
            print(
                f"{args.targets} file extension unrecognized, for expected behavior pass GFF/GTF/BED file.\n \
    ...
    if args.ranking_columns:
        finalgRNAs = create_combined_weighted_column(
            df=finalgRNAs,
            column_names=args.ranking_columns,
            weights=args.column_weights,
    ...
    # Filter by spacing
    out_path = tmp_path + str(i) + "_gRNAsTargets_spacing" + "Filter.bed"
    i += 1
    if args.ranking_columns:
    ...
    if "id,sequence,pam,chromosome,position,sense" in finalgRNAs.columns:
        finalgRNAs[["sequence"]] = (
            finalgRNAs["id,sequence,pam,chromosome,position,sense"].str.split(",", expand=True).iloc[:, 1:2]
        )
    ...
    if args.plot_histogram:
        create_histogram(
            finalgRNAs,
            gRNA_output_path[:-3] + "pdf",
            args.target_mode,
            update_no_targets(initial_target_ids, finalgRNAs),
        )

    save_notarget_set(
        noSgRNATargetSet=targetsWithoutgRNAs.union(update_no_targets(initial_target_ids, finalgRNAs)),
