...
from crisprware.utils.gtf_bed_processing_functions import (
    ...
    gtf_to_tss_tes_bed,
    parse_line,
    extract_transcript_gene_relationship,
    parse_input,
    create_metagene_model,
    create_constitutive_model,
    truncate_gtf,
)
import pandas as pd
...
class TestGTFBEDFunctions(unittest.TestCase):
    ...
    def test_gtf_to_tss_bed(self):
        ...
        result_TSS, result_TES = gtf_to_tss_tes_bed(
            input_gtf,
            tss_upstream=tss_upstream,
            tss_downstream=tss_downstream,
            tes_upstream=tes_upstream,
            tes_downstream=tes_downstream,
        )
...
class TestParseInput(unittest.TestCase):
    def test_parse_input(self):
        ...
        expected_genes = {
            "gene1": {
                "exons": {"transcript1": [(20, 35), (40, 50)]},
                "CDS_coords": {"transcript1": [(25, 30)]},
                "CDS": set(["transcript1"]),
                "strand": "+",
                "chromosome": "test_chr",
            },
            "gene2": {
                "exons": {"transcript2": [(120, 140), (150, 170)]},
                "CDS_coords": {"transcript2": [(130, 140), (150, 160)]},
                "CDS": set(["transcript2"]),
                "strand": "+",
                "chromosome": "test_chr",
            },
            "gene3": {
                "exons": {"transcript3": [(200, 230), (240, 260)], "transcript4": [(200, 230), (240, 260)]},
                "CDS_coords": {"transcript3": [(211, 230), (240, 251)], "transcript4": [(211, 230), (240, 251)]},
...
class TestCreateConstitutiveModel(unittest.TestCase):
    def test_create_constitutive_model(self):
        ...
        with open(output_file, "r") as f:
            expected_output_str = f.read().strip()
...
class TestCreateMetageneModel(unittest.TestCase):
    ...
    def test_create_full_a2b1_model(self):
        # a2b1 coding transcripts
        input_file = "./tests/test_data/Hnrnpa2b1.gtf"
...
"""
    ...
    def test_truncate_gtf_no_truncation(self):
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_1), feature="exon", percentiles=[0, 100]
        )
        ...
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_1), feature="exon", percentiles=[0, 50]
        )
        ...
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_1), feature="exon", percentiles=[10, 20]
        )
        ...
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_2), feature="CDS", percentiles=[25, 75]
        )
        ...
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_3), feature="exon", percentiles=[0, 100]
        )
        ...
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_3), feature="exon", percentiles=[0, 50]
        )
        ...
        df, unique_transcript_ids, unique_gene_ids = truncate_gtf(
            StringIO(self.gtf_content_4), feature="exon", percentiles=[25, 75]
        )
